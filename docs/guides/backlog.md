...
v1.17 已修复活动 SSE 订阅者在事件历史滑窗裁剪后停止收流的问题：`TaskBroker` 为事件分配单调递增的 `seq`，活动订阅者通过独立队列继续接收事件。但浏览器网络断线后的游标续传尚未实现：服务端没有消费 `Last-Event-ID` 或显式 `after_seq`，前端也会在 `EventSource.onerror` 时立即关闭连接。

实施时至少覆盖：

- 服务端用标准 SSE `id` + `Last-Event-ID`，或显式 `after_seq` 参数恢复游标。
- 前端只在终态或主动切换作用域时关闭连接；可恢复网络错误应重连，并按 `seq` 去重。
- 游标仍在重放窗口内时不得重复或遗漏任何事件，尤其不能重复追加 `token`。
- 游标已经落后于窗口时必须返回可识别的缺口状态，引导前端等待任务完成后刷新持久化会话，不能静默拼接残缺回复。
- 覆盖活动连接超过窗口、断线重连、重复连接、窗口缺口、终态送达和助手作用域隔离测试。
