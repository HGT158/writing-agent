...
2. `docs/architecture/phase1-architecture.md`：架构设计 v1.23，项目单一事实来源。
3. `README.md`：当前安装、运行和 Scheduler 使用说明。

项目当前状态：

- 阶段 1 架构、阶段 2 MVP、阶段 3 Memory + Scheduler、阶段 4 写作工作台均已完成。
- 当前完整测试基线是 **pytest 225/225 全绿**；记忆隔离红线为 10/10；前端测试为 102/102，类型检查与生产构建通过。
...
- 架构文档当前版本是 v1.23；除阶段 4 复审与 v1.13–v1.22 外，还完成了 phase7 复审加固：工作记录脱敏整体修复（字符串参数不再绕过）、失败详情值级脱敏与截断、明细落库失败降级不打断任务、reject 前清理孤儿写意图、CodeMirror 语法高亮真正主题化、连续对话保留上一轮工作记录、侧栏卡片点击打开目标文档。当前没有自动开始的新阶段。
