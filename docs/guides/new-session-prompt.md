...
- 架构文档当前版本是 v1.17；除阶段 4 复审与 v1.13–v1.16 外，还完成了活动 SSE 连接按 `seq` 跨越事件滑窗、编辑器内联 diff 与侧栏卡片双视图、选区工具栏可输入、项目聊天上下文分层压缩以及前端助手增删。SSE 网络断线后的游标续传尚未实现，已列入待办；当前没有自动开始的新阶段。
