...
2. `docs/architecture/phase1-architecture.md`：架构设计 v1.25，项目单一事实来源。
...
- 架构文档当前版本是 v1.25；除阶段 4 复审与 v1.13–v1.24 外，还完成了 v1.25 资源管理器整理：项目/文件树形嵌套渲染（VS Code 式多根目录 + 同级名称交错排序）、项目与文件行内重命名/删除按钮（文档重命名与删除为新增后端契约，含路径校验、冲突与待处理建议拒绝、入口文档改指）。当前没有自动开始的新阶段。
