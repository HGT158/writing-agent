...
2. `docs/architecture/phase1-architecture.md`：架构设计 v1.24，项目单一事实来源。
3. `README.md`：当前安装、运行和 Scheduler 使用说明。

项目当前状态：

- 阶段 1 架构、阶段 2 MVP、阶段 3 Memory + Scheduler、阶段 4 写作工作台均已完成。
- 当前完整测试基线是 **pytest 226/226 全绿**；记忆隔离红线为 10/10；前端测试为 108/108，类型检查与生产构建通过。
...
- 架构文档当前版本是 v1.24；除阶段 4 复审与 v1.13–v1.23 外，还完成了 phase8 复审加固：值级脱敏切片边界修复（key=value 长前缀形态曾完整泄漏敏感值）、明细落库失败降级 warning 补发配对 start 事件实时可见、hunk 定位回退搜索；同版修复实测缺陷：change_preview 载荷补 hunk status（不再报"无效的修改预览"）、运行中耗时自动跳动、外部内容同步保持滚动位置（不销毁重建编辑器 + 最小差异区间替换）、失效建议可从侧栏放弃关闭。当前没有自动开始的新阶段。
