...
> v1.31 变更：TRAE 式模型/提供商切换（backlog 2026-08-22 登记项；方案经用户确认：独立本地配置文件存密钥、按任务路由、温度按提供商统一）。**（1）多提供商配置存储**：新增 `agent/llm_providers.py` 的 `LLMProviderStore`，全部提供商（id/显示名/base_url/api_key/可用模型列表/温度）与当前选择持久化于项目根目录 `llm_providers.json`（与 `.env` 同目录，已加入 `.gitignore`，白盒可手改）；写入走「临时文件 + 原子替换」并尽力收紧文件权限（POSIX `chmod 0600` / Windows `icacls` 限当前用户与 SYSTEM，失败仅记日志不阻断）。首次启动从 `.env` 的 `OPENAI_API_KEY/OPENAI_BASE_URL/MODEL_NAME` 合成 `default` 提供商并落盘（一次性迁移，`.env` 此后仅作首次引导占位）；文件缺失即按引导重建 default，文件损坏（JSON 解析失败、`current` 指向未知提供商或未声明模型）在 Runtime 启动时显式报错并指向文件路径，不静默回退。**（2）按任务路由与运行期切换**：Runtime 持有提供商注册表、按 `base_url + api_key` 复用的 `AsyncOpenAI` client 缓存与当前选择指针；每个任务（`run` / `chat_project` / `rewrite_selection`）在获锁前解析一次「当前提供商 → (client, model, temperature)」作为本任务快照，任务内全部 LLM 调用（含历史压缩与记忆沉淀提取）使用同一快照；切换当前提供商 = 原子更新指针并持久化，只影响后续任务，不打断运行中任务。切换/新增不占用助手运行锁——提供商是全局基础设施而非助手数据（§2 共享边界）；`runtime.llm` 保持为「当前提供商客户端」的属性门面，注入的测试替身经 setter 覆盖仍生效。**（3）温度配置化（phase6 遗留闭环）**：温度成为提供商配置项（`temperature`，0–2，缺省 0.3），`llm.py` 与各调用点（原 Planner 0.3 / 大纲 0.5 / 成文 0.6 / 质检与压缩与记忆提取 0.2 / 选区改写 0.3）的硬编码常量收敛为统一使用当前提供商温度——节点级差异取消（取舍经用户确认；多提供商后按厂商统一调节更可控）。**（4）API**：`GET /api/llm/providers`（列表 + 当前选择；`api_key` 永不下发原文，只回掩码尾缀）、`POST /api/llm/providers`（新增，校验后原子落盘并返回刷新后的列表载荷）、`POST /api/llm/providers/current`（切换当前提供商与模型；未知提供商 404、未声明模型 400）。**（5）前端**：Agent 面板输入区新增模型选择按钮与下拉菜单（按提供商分组、勾选当前项、方向键/Home/End 导航对齐主题菜单交互），二级入口「添加提供商」对话框（显示名/base_url/API Key/可用模型每行一个/可选温度）；保存成功后刷新列表并自动切换到新提供商的第一个模型，服务端拒绝原样提示且不关闭对话框。「可用性测试按钮」（"你好"探活）为后续增强，本版不做。**（6）密钥边界**：AGENTS.md 硬性规则修订为「密钥只能来自 `.env` 或项目根目录受管的 `llm_providers.json`，禁止硬编码和提交」；`.env.example` 中 `OPENAI_*` 标注为首次引导占位。
> v1.32 变更：纯文档口径修正（phase10 复审，不改任何代码行为）——§5.7 工作记录截断标注口径改「脱敏后长度标注」（对齐 v1.29 实现，原正文误写「原始长度标注」）；§5.4 工作记录段落补终态守卫契约（置位终态后 `start`/`note` 显式拒绝，v1.29 行为入文）；§9 聊天记忆沉淀失败语义如实化（提取调用失败时画像保持原状；`memorize` 为逐条写入，中途失败时已写入条目保留、其余跳过，原误写「画像保持原状」一刀切）；v1.31 两处措辞更正（任务快照解析先于运行锁获取；Windows `icacls` 收紧范围含 SYSTEM）。phase10 审查报告见 `docs/reviews/phase10-code-review.md`（v1.28–v1.31 四提交区间复审）。
> v1.33 变更：phase10 复审代码侧修复（处理报告全部 P1/P2 与除 backlog 观察项外的 P3；架构文档随本次修复一次升版 v1.33，逐条处置见 `docs/reviews/phase10-code-review.md` 文末「处理结果记录」）。**（1）助手编辑契约修正（P1-1/P3-1）**：`AssistantUpdate.description` 撤销 v1.30 单边加入的 `min_length=1`，空串语义与创建对齐（原状态下空描述助手在 UI 中无法保存任何编辑且提示为不可读的状态码文本）；PATCH 显式 `null` 字段由端点检测返回 422（列出字段名），不再静默当作「未提供」的 200 no-op；前端统一请求层对 FastAPI 校验错误（422 数组 `detail`）取首条 `msg` 作为可读提示。**（2）提供商配置写入收口（P2-1/P3-16/P3-17/P3-18/P3-19/P3-20）**：`LLMProviderStore` 变更+落盘以进程内 `RLock` 串行（API 写端经 `asyncio.to_thread` 在工作线程执行，前端快速连点可产生并发 in-flight 切换）；`_flush`/`payload`/`selection`/`resolve` 对当前选择取单一快照引用，并发切换不可能把交叉 `(provider, model)` 配对写进文件（该配对原会让下次启动直接 RuntimeError）；落盘顺序改为「先落盘成功、再更新内存」，flush 失败时内存与磁盘不分叉；写临时文件后 `fsync` 再 `os.replace`，启动时清扫超过 60 秒宽限期的孤儿 `.llm_providers-*.tmp`（含明文 Key，宽限期内保留给并发进程）；短密钥（<16 位）整体掩码；`base_url` 升级为 urlparse scheme+netloc 结构校验并拒绝空白字符；白盒手改文件的值级错误（温度越界、模型名空白）包装为带文件路径的 `RuntimeError`，models 复用既有清洗（去空白、去重）。**（3）记忆管线收口（P1-2/P1-3/P2-6/P2-7/P2-8 代码侧/P3-11/P3-13）**：直达沉淀有界化——显式指令内容超过 `MAX_ITEM_CHARS`（120 字）或含换行不直达（交启发式提取由模型裁决，避免截断产生无意义片段），疑问语气（结尾 吗/呢/？/?）不直达；直达与启发式两条路径落库前均与画像既有内容做规范化（去空白）比对去重，同一句话重复说只落一条；注入记忆块按 `chat_context_token_budget` 的 1/3 份额裁剪（chat 与普通 run 双路径，复用 `clip_content_to_token_budget`，预算为 0 即关闭压缩时不裁剪），记忆裁剪与「文档因预算不足被整体挤出」都以 warning 条目与 `warning` 事件指明原因（提示可经「记忆画像」对话框清理）；提取调用 prompt 只携带画像前 8,000 字符；提取调用以 `chat_memory_extraction_timeout_seconds`（默认 30 秒，`CHAT_MEMORY_EXTRACTION_TIMEOUT_SECONDS` 可配）独立限时，最坏从无界（约 120–240 秒）收敛到 30 秒内——选择「有界同步」而非「锁释放后 create_task 分离」：分离会让沉淀条目出现在任务终态之后（实时流已关闭、只能刷新可见）且需给记录器引入跨实例序号续接，契约收益不抵复杂度；`memorize` 逐条写入，中途失败时已写入条目保留并以进度条目呈现，警告文案如实为「本轮记忆沉淀部分失败：已写入 k/N 条，其余跳过」；回复已交付并落库后提取中的 `CancelledError` 由沉淀函数吞掉并记 info 日志，不再把本轮改记 interrupted；画像编码损坏（白盒手改存成 ANSI/GBK）时 `replace_profile` 按无原文覆盖修复，API GET 映射为带指引的 400；普通任务启动只在记忆命中或降级时播报「已注入助手记忆」，零命中静默。**（4）助手文件一致性（P2-2/P2-3/P2-4/P2-5/P3-12）**：`assistant.yaml`/`persona.md`/`profile.md` 的全部写入（create、update、回滚、画像整文替换/首部初始化/超限裁剪重写）统一改同目录临时文件 + fsync + `os.replace` 原子替换——进程中断或断电不再留下截断/空文件，GET /memory/profile（不取运行锁的只读端点）与写入并发只读到旧文件或新文件；registry `reload()` 整体重建后单次替换 `_assistants`、`get` 改单次字典查找，进程内 `RLock` 串行写端与 reload——PATCH/create/delete（工作线程）与读端（事件循环线程）并发时不再出现存在的助手瞬时 404、列表瞬时缺项；更新写入失败且回滚也失败时改经 `logging.warning` 记录（含助手 id 与文件路径）——原 `self.warnings` 会被任何后续 `reload()` 清空且启动后无人消费；显示名在 registry 层统一收口：空白（全空格）拒绝，CLI 与 API 同口径。**（5）边界收尾（P3-2/P3-4/P3-8/P3-9，后两项为契约修订）**：persona 50,000 字符上限下沉 `AssistantRegistry.create/update`（原仅 API Pydantic 层校验，CLI 直通可写入数 MB persona），CLI `--persona-file` 非 UTF-8 文件给可读报错；`persona_file` 键只允许指向助手目录之内（绝对路径与 `../` 越界按损坏配置拒绝，update 400、reload 记 warning 跳过不阻断启动）；`WorkLogRecorder.finish_task` 终态旗标移到终态行成功落库之后置位（落库中途失败时补偿 `finish_task` 可重试，不再被幂等守卫吞掉留下「任务报 failed 但工作日志无终态行」窗口；finish_task 全程同步执行，旗标后置不产生 start/note 竞态窗口，v1.29 守卫语义不变）；`_finalize_write_intent` 意图缺失分支由「返回空列表」（phase7 P3-5 引入）改为抛 `StorageRecoveryPendingError`（API 409）——静默成功会造成「磁盘已写、DB 未推进」的假成功：accept 报 200 而 hunk 仍 pending、无任何告警；该分支需复合竞态才可达，正常路径不受影响。**（6）前端（P3-5/P3-22）**：模型切换请求 in-flight 期间禁用选择器触发器；「添加提供商…」纳入方向键/Home/End roving 循环；助手对话框 persona 输入显示「N / 50000」字数计数（maxlength 静默截断粘贴的知情补偿）；422 数组 detail 取首条 `msg` 呈现。
...
- **保留窗口总量兜底（v1.21）**：超预算的原因是保留窗口自身过大（无更早历史可压缩，或压缩后仍超）时，先对进入 prompt 的单条消息按"预算 60%、至少 1000 字符"的上限做首尾窗口截断（复用文档截断标记），再从最旧开始收缩窗口，最新一条消息单独超预算时同样截断——prompt 估算恒不超预算。兜底产生的截断/丢弃都记入 warnings；只影响 prompt，不影响可见历史与 `propose_project_edits` 的服务端精确匹配。
...
persona_file: persona.md               # 可选；只能指向助手目录之内，越界按损坏配置拒绝（v1.33）
created_at: "2026-08-05"
```

persona 系统提示词上限 50,000 字符，registry 层单点收口——CLI 直通与 API 同口径，超限拒绝（v1.33）。
...
**编辑语义（v1.28，v1.33 加固）**：显示名、描述与系统提示词可经 API 编辑，采用 PATCH 部分更新语义——请求中未提供的字段保持不变，显式 `null` 拒绝（422）。`assistant.yaml` 重写时保留 `skills`、`created_at`、`persona_file` 等既有字段；persona 写入 `persona_file` 指向的文件（缺省 `persona.md`，UTF-8）。助手定义与 persona 的全部写入（含回滚）走同目录临时文件 + fsync + 原子替换，进程中断或断电不留截断文件。显示名在 registry 层统一收口：空白（全空格）拒绝，CLI 与 API 同口径。更新与删除使用同一边界：先获取助手运行锁（§4.6），该助手任一任务运行期间拒绝（409）；磁盘写入失败时按内存中的原内容尽力回滚，不留下半程状态，回滚也失败时经日志告警（运行期可见）。创建或更新请求提供的 `persona` 为空白时一律落为默认 persona；`persona.md` 被人手工清空属显式选择，reload 按文件事实返回，不做静默改写。`default` 助手可编辑、不可删除。
...
项目支持新建空白项目（默认创建 `article.md`）、重命名显示名和归档删除。归档将整个项目目录移动到 `data/archive/projects/<assistant_id>/<project_id>-<UTC 时间戳>/`，不复用项目 id；运行中、存在未应用 change set 或项目内存在活跃写入意图时拒绝归档。物理清除只允许显式 `purge=true`，且同样必须先拒绝活跃写入意图。MemoryStore 启动时必须对账项目级崩溃残骸：未注册的 `.import-*` staging 可删除；`.purge-*` 在项目元数据仍存在时恢复到登记位置、元数据已删除时清除；活动项目目录缺失但存在唯一同 id 归档半程目录时移回活动目录。每个正式项目目录和 import staging 都携带内部管理标记，标记至少包含固定格式版本、`assistant_id` 与 `project_id`；对账只有在标记存在、可解析且身份与当前受管路径一致时才允许删除，未知或损坏标记一律保留供人工核查。目录最后修改时间距对账时不足五分钟时跳过，覆盖另一进程正在创建、导入或 purge 的窗口；purge 移动前刷新目录时间，事务提交后的 staging 删除按幂等清理处理。对账只处理受管目录和可由数据库唯一归属的路径。
...
**模型提供商按任务路由（v1.31）**：Runtime 持有 `LLMProviderStore`（`agent/llm_providers.py`）——全部提供商与当前选择（提供商 id + 模型名）持久化于项目根目录 `llm_providers.json`，与 `.env` 同目录、已 gitignore、白盒可手改；写入走「临时文件 + fsync + 原子替换」，并尽力收紧文件权限（POSIX `chmod 0600` / Windows `icacls` 限当前用户与 SYSTEM，失败仅记日志不阻断）；启动时清扫超过 60 秒宽限期的孤儿 `.llm_providers-*.tmp`（含明文 Key，宽限期内的临时文件可能属于并发进程在写，保留不动）。首次启动文件不存在时，从 `.env` 的 `OPENAI_API_KEY/OPENAI_BASE_URL/MODEL_NAME` 合成 `default` 提供商并落盘（一次性迁移，`.env` 此后仅作首次引导占位）；文件损坏（JSON 解析失败、`current` 指向未知提供商或未声明模型、提供商 id 重复、温度越界或模型名空白）在启动时显式报错并指向文件路径，不静默回退。每个任务（`run` / `chat_project` / `rewrite_selection`）在获锁前解析一次「当前提供商 → (client, model, temperature)」作为本任务快照：client 按 `base_url + api_key` 缓存复用，任务内全部 LLM 调用（含历史压缩与记忆沉淀提取）使用同一快照。切换当前提供商 = 原子更新指针并持久化，只影响后续任务，不打断运行中任务；切换/新增不占用助手运行锁（提供商是全局基础设施而非助手数据，§2 共享边界）。进程内的变更+落盘以 `RLock` 串行（API 写端经 `asyncio.to_thread` 在工作线程执行），对当前选择一律取单一快照引用——并发切换不可能把交叉 `(provider, model)` 配对持久化；新增/切换先落盘成功再更新内存，落盘失败时内存与磁盘不分叉。新增提供商的 `base_url` 在保存期做 urlparse scheme+netloc 结构校验并拒绝空白字符。当前提供商未配置 API Key 时任务以「未配置 OPENAI_API_KEY」语义报错拒绝启动，提示经界面添加提供商或编辑 `.env`（与既有缺钥行为对齐）。`runtime.llm` 保留为「当前提供商客户端」的属性门面（测试替身经 setter 注入仍生效），但任务内一律消费启动时的解析快照。**温度配置化（v1.31，phase6 遗留闭环）**：温度是提供商配置项（`temperature`，0–2，缺省 0.3），全部 LLM 调用点统一使用当前提供商温度，原 Planner/大纲/成文/质检等节点级硬编码差异取消。
...
**项目聊天注入本助手记忆与轮次终态选择性沉淀（v1.30，v1.33 收口）**：`chat_project` 组装 system prompt 时在编辑指导之前注入本助手记忆——`recall_trace(assistant_id, message)` 的画像、命中历史文章索引与相关对话片段（§4.7「聊天可读取本助手记忆」的实现补齐）；注入内容属于 system prompt 的一部分，参与既有 token 预算与 v1.21 兜底计算。**注入块自身按 `chat_context_token_budget` 的 1/3 份额裁剪**（复用 `clip_content_to_token_budget`；预算为 0 即关闭压缩时不裁剪）——大画像不得把文档正文挤成空串或清空历史窗口；裁剪发生与「文档因预算不足被整体挤出」时都以 warning 条目与 `warning` 事件指明原因。注入动作在工作记录落一条 progress 条目（「已注入助手记忆」，detail 携带画像条数、文章命中数、对话片段数与分路降级标记；普通任务启动仅在命中或降级时播报同类 info，零命中静默）。
...
2. **显式指令直达（零模型调用）**：消息匹配显式指令且指令之外有正文时，剥离指令词直接 `memorize(kind="preference", content=正文, session_id=chat_session_id)`，不调模型。直达有边界（v1.33）：正文超过 120 字或含换行不直达（交启发式提取由模型裁决，避免截断产生无意义片段）；疑问语气（结尾 吗/呢/？/?）不直达；与画像既有条目做规范化（去空白）整条相等比对，重复时不写入——不做跨条目子串包含，短新偏好不被更长的既有条目误判为重复（v1.33 复审补强）。
3. **启发式提取（每轮至多一次模型调用）**：其余命中场景用一次非流式 JSON 提取调用（输入=当轮 user/assistant 消息与画像截取片段（前 8,000 字符，v1.33）；输出 ≤3 条 `{kind, content}`，kind ∈ preference/style/topic，content 为 ≤120 字的一句话陈述；提示词要求与画像已有等价记录去重，一次性编辑指令、事实问答与寒暄不沉淀），对每条输出做规范化去重后调用 `store.memorize`。提取调用有独立短超时（`CHAT_MEMORY_EXTRACTION_TIMEOUT_SECONDS`，默认 30 秒，v1.33），超时按提取失败降级。无输出则静默结束，不产生工作记录条目。

沉淀动作落一条 progress 条目（「已沉淀助手记忆」，detail 列出条目与 kind）；提取调用失败（含超时）未写入任何条目，警告文案为「本轮记忆沉淀失败：未写入任何条目」；`memorize` 为逐条写入，中途失败时已写入条目保留并以进度条目呈现，警告文案如实为「本轮记忆沉淀部分失败：已写入 k/N 条，其余跳过」（v1.33）——两者都记 warning 日志并在工作记录落警告条目，**不得影响本轮已交付的聊天回复**（对齐 §3.3 压缩失败的降级语义）。回复已交付并落库之后，提取中的取消只放弃沉淀本身，不把本轮改记 interrupted（v1.33）。failed/interrupted 终态不沉淀。`CHAT_MEMORY_CONSOLIDATION`（默认开启）可整体关闭该行为。普通写作任务的 recall（`runtime.run` 启动时一次）同样按预算份额裁剪注入，并在命中时以 `info` 事件播报命中摘要（画像条数、文章命中数、对话片段数、降级标记、截断标记），注入内容形态不变。
...
**启动超时与资源晋升（v1.27）**：每个 server 的 stdio transport 建立、`ClientSession.initialize()` 与 `list_tools()` 三步分别受 10 秒超时约束，任一步超时或失败只让该 server 降级。连接过程先使用 server 私有的临时 `AsyncExitStack`；只有三步全部成功后才把该栈的清理回调转移到 manager 长命栈并注册工具，失败路径立即 `aclose()` 临时栈，确保已拉起的子进程和管道不会滞留到应用关闭。manager 关闭时只清理由成功连接晋升的 session。
...
- **画像白盒读写与结构化命中（v1.30）**：
  - `get_assistant_profile(assistant_id) -> str` / `replace_assistant_profile(assistant_id, content) -> None`：整文读写 `profile.md`，UTF-8 原样写入、不重排既有格式、不自动补写头部；`content` 超过 `ASSISTANT_PROFILE_MAX_CHARS`（50,000 字符）抛 `ValueError`，空白内容属显式清空、原样落盘；写入走临时文件 + fsync + 原子替换（v1.33），失败按原内容尽力回滚，不留下半程状态（与 persona 编辑同模式）。读写仍只经 MemoryStore，前端只能经 API 访问（§5.9），禁止直接读助手目录。
...
| `PATCH /api/assistants/{assistant_id}` | 编辑助手（v1.28）：显示名、描述与系统提示词的部分更新，仅提供的字段生效，全部缺省返回 400；显式 `null` 字段返回 422（与「未提供」区分，v1.33）；`description` 允许空串，语义与创建一致（v1.33）；空白 `persona` 落为默认 persona；以助手运行锁串行，该助手任一任务运行期间返回 409，边界与删除一致；未知助手 404 |
| `GET /api/assistants/{assistant_id}/memory/profile` | 返回本助手长期画像 `{content}`（v1.30）；未知助手 404；profile 文件不存在返回空串；文件编码损坏（非 UTF-8）返回带指引的 400（v1.33） |
| `PUT /api/assistants/{assistant_id}/memory/profile` | 整文替换长期画像（v1.30）：body `{content}` 上限 50,000 字符，UTF-8 原样写入、空白即显式清空；原文件编码损坏时按覆盖修复继续写入（v1.33）；先取助手运行锁，该助手任一任务运行期间返回 409（与 PATCH assistant 同边界）；超上限 400，未知助手 404 |
| `GET /api/llm/providers` | 模型提供商列表与当前选择（v1.31）：`{current: {provider_id, model}, providers: [{id, name, base_url, models, temperature, api_key_hint}]}`；`api_key` 永不下发原文，只回掩码尾缀（如 `sk-***abcd`；不足 16 位的短密钥整体掩码 `***`，v1.33），未配置回空串 |
| `POST /api/llm/providers` | 新增提供商（v1.31）：body `{name, base_url, api_key, models, temperature?}`；校验失败 422/400（base_url 须 http(s) 且带主机名、不含空白字符，v1.33 收紧；models 非空且去重、温度 0–2）；id 服务端生成，成功原子落盘并返回刷新后的列表载荷 |
...
**客户端停滞看门狗（v1.27）**：所有普通 fetch 请求统一在 60 秒后 abort，并把超时转换为可读错误，避免 busy 状态永久占用。`watchTask` 在每次 EventSource `onopen` 后启动 60 秒空闲计时；服务端任意数据帧或每 15 秒发送的命名 `heartbeat` SSE 事件均视为活动并重置计时，heartbeat 只服务连接存活检测、不向业务组件派发。60 秒内没有任何事件时主动关闭当前 EventSource，并复用既有 `lastSeq`、退避序列、去重和缺口恢复逻辑重连。显式 close、终态或解析失败必须同时清理连接、退避与看门狗计时器。

API 层不通过错误文本猜测冲突类型。MemoryStore/项目存储以专用冲突异常表达版本冲突、待处理状态和跨进程写入占用，API 稳定映射为 HTTP 409；参数错误保持 400，资源不存在保持 404。

### 5.10 前端 — `web/`（阶段 4）

Vue 3 + Vite 单页采用 VS Code 式写作 IDE，而非聊天主界面：顶部/活动栏提供**助手选择器**和项目导入；左侧资源管理器列出当前助手的项目及项目文件树，活动项目的文件树紧贴该项目行渲染、把后续项目行推下去（多根目录行为），项目内子文件夹为可展开/收起的文件夹行、同级按名称交错排序、文件行只显示文件名（悬停提示完整相对路径）；中间为多标签 CodeMirror 编辑器，可切换 Markdown 预览；右侧为绑定当前项目的 Agent 面板，显示对话、执行事件、来源和待确认修改。切换助手时必须关闭或重新加载不属于新助手的项目标签和 Agent 会话。
...
**Agent 面板输入区模型选择（v1.31）**：composer 新增模型选择按钮，文案显示当前「提供商 · 模型」；下拉菜单按提供商分组列出可用模型、勾选当前项，交互对齐主题菜单——打开聚焦当前项、方向键循环导航、Home/End 跳首尾、Esc/点击外部关闭，「添加提供商…」二级入口同属该 roving 循环（v1.33）。选中菜单项即调用 `POST /api/llm/providers/current` 切换并更新按钮文案，切换请求 in-flight 期间禁用选择器触发器（v1.33，与后端锁共同串行化），切换失败在面板内原样提示且保持原选择。菜单底部「添加提供商…」二级入口打开对话框：显示名、base_url、API Key、可用模型（每行一个）与可选温度（0–2，缺省 0.3）；提交调用 `POST /api/llm/providers`，成功后刷新列表并**自动切换**到新提供商的第一个模型；服务端拒绝（422/400）原样提示且不关闭对话框、保留已填内容——统一请求层把 FastAPI 校验错误（422 数组 `detail`）取首条 `msg` 呈现为可读消息，不退化成状态码文本（v1.33）。提供商列表经 `GET /api/llm/providers` 获取并本地缓存于 AgentPanel，切换/新增后以响应载荷刷新；前端任何时候拿不到 API Key 原文，只显示掩码尾缀；配置本体在项目根目录 `llm_providers.json`（不入 git）。「可用性测试按钮」（"你好"探活）为后续增强，本版不提供。
...
- **侧栏卡片视图**：AgentPanel 始终列出本会话全部含 pending/stale hunk 的 change set，卡片展示目标文件相对路径与 hunk 摘要（各 hunk 状态），并提供"全部接受/全部放弃"批量入口（全部接受 = 服务端倒序串行应用）。目标文档未打开时，侧栏是唯一入口；点击卡片打开目标文档，点击具体 hunk 滚动定位。**hunk 定位回退（v1.24）**：内联装饰不可用（标签页 dirty、hunk 已非 pending 或内容重定位失败）时，按该 hunk 的原文在当前正文搜索回退定位并选中命中文本；原文已不在正文中则给出轻提示交由侧栏处理，不得静默无操作。
...
| MCP server 建连/初始化/工具发现停滞 | 每步 10 秒超时；关闭该 server 临时 exit stack 与子进程，记 warning 后继续启动其他 server |
...
| 编辑运行中的助手（PATCH） | 先取助手运行锁，任一任务运行期间拒绝（409），边界与删除一致；写失败按原内容尽力回滚，不留下半程状态，回滚也失败时经日志告警（v1.33）；写入走临时文件 + fsync + 原子替换（v1.33）（§4.2/§5.9） |
| 助手 PATCH 未提供任何字段或字段非法 | 400；显式提供 `null` 字段返回 422（v1.33）；空白显示名在 registry 层拒绝、两端同口径（v1.33）；未知助手 404；创建/更新提供的空白 persona 落为默认 persona，不产生空系统提示词（§4.2） |
| 助手目录损坏（YAML 解析失败） | 该助手标记不可用并 warning，不影响其他助手与系统启动 |
| persona_file 指向助手目录之外（手改配置，v1.33） | update 拒绝（400）；reload 按损坏配置记 warning 并跳过该助手，不阻断启动 |
| persona 超过 50,000 字符（CLI 与 API 同口径，v1.33） | registry 层拒绝：API 400 / CLI 报错 |
| CLI --persona-file 非 UTF-8 文本（v1.33） | 可读报错「persona 文件必须是 UTF-8 文本：<路径>」，不创建半程助手 |
| 工作日志终态行落库失败 | 原始错误照常抛出；补偿 finish_task 可重试（终态旗标在成功落库后置位，v1.33），仍失败只记 warning、不得掩盖原始任务错误 |
...
| 前端保存/apply 请求在途时标签页继续编辑 | 响应回写前同时核对发起时版本与正文指纹；任一变化则保留本地正文和 dirty 状态并提示冲突，不用服务端快照覆盖 |
| 项目 archive/purge 遇到活跃文档写入意图 | 移动目录前返回写入冲突；活动项目先恢复死进程遗留意图，仍存续的真实在途写者不得被归档或清除 |
| 启动对账遇到无标记、标记身份不符或五分钟内新建的目录 | 一律跳过且不删除；只有旧的、标记完整且身份与受管路径一致的残骸可按数据库状态清理/恢复 |
...
| 服务端持续接受 SSE 连接后未送达任何事件即断开（开连即断） | `onopen` 不再复位重连退避，复位以收到首个事件（数据帧或 heartbeat）为条件；重连尝试照常累计，6 次耗尽后订阅关闭并提示刷新恢复，不产生无界快速重连（§5.10） |
...
| 聊天记忆提取调用失败或 memorize 写入失败 | 降级 warning：记日志并在工作记录落警告条目；本轮聊天回复不受影响。提取调用有独立短超时（默认 30 秒，v1.33），失败时画像保持原状、文案为「未写入任何条目」；`memorize` 为逐条写入，中途失败时已写入条目保留、其余跳过，警告如实标注「已写入 k/N 条」，无部分提交回滚（v1.32/v1.33 如实化） |
| 回复已交付后沉淀提取期间被取消 | 只放弃沉淀本身并记 info 日志，本轮照常 succeeded 终态，不改记 interrupted（v1.33） |
| 注入记忆超出预算份额 / 当前文档因预算不足被挤出 | 注入块按 `chat_context_token_budget` 的 1/3 裁剪（复用 clip_content_to_token_budget）；两种情形都以 warning 条目与 `warning` 事件指明原因（v1.33） |
| profile 整文替换超上限 / 助手任务运行中 / 原文件编码损坏 | 400 / 409 / 覆盖修复（编码损坏按无原文处理继续写入，v1.33）；GET 编码损坏返回带指引的 400；写入失败按原内容尽力回滚，不留下半程状态（§5.7/§5.9） |
...
| 重命名文档的目标路径非法、扩展名不可编辑或作用于只读文件 | API 返回 400，磁盘与元数据不变（v1.25） |
...
| `llm_providers.json` 损坏（JSON 解析失败、current 指向未知提供商/未声明模型、id 重复、温度越界或模型名空白——v1.33） | Runtime 启动显式报错并指向文件路径，不静默回退；人工修复或删除该文件后重启自动重建 |
| 切换到未知提供商或未声明模型 | API 404 / 400，当前选择保持不变 |
| 当前提供商未配置 API Key | 任务拒绝启动，错误语义与原「未配置 OPENAI_API_KEY」一致，提示经界面添加提供商或编辑 .env |
| 新增提供商参数非法（base_url 非 http(s) 或缺主机名/含空白——v1.33 收紧、models 空、温度越界） | 422/400 原样返回；磁盘与既有提供商不变 |
| 提供商新增/切换落盘失败（磁盘故障等，v1.33） | 请求报错；内存状态不变（先落盘成功再更新内存），重试不产生半程状态 |
