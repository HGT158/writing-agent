...
**编辑语义（v1.28，v1.33 加固）**：显示名、描述与系统提示词可经 API 编辑，采用 PATCH 部分更新语义——请求中未提供的字段保持不变，显式 `null` 拒绝（422）。`assistant.yaml` 重写时保留 `skills`、`created_at`、`persona_file` 等既有字段；persona 写入 `persona_file` 指向的文件（缺省 `persona.md`，UTF-8）。助手定义与 persona 的全部写入（含回滚）走同目录临时文件 + 原子替换，进程中断不留截断文件。显示名在 registry 层统一收口：空白（全空格）拒绝，CLI 与 API 同口径。更新与删除使用同一边界：先获取助手运行锁（§4.6），该助手任一任务运行期间拒绝（409）；磁盘写入失败时按内存中的原内容尽力回滚，不留下半程状态，回滚也失败时经日志告警（运行期可见）。创建或更新请求提供的 `persona` 为空白时一律落为默认 persona；`persona.md` 被人手工清空属显式选择，reload 按文件事实返回，不做静默改写。`default` 助手可编辑、不可删除。
...
2. **显式指令直达（零模型调用）**：消息匹配显式指令且指令之外有正文时，剥离指令词直接 `memorize(kind="preference", content=正文, session_id=chat_session_id)`，不调模型。直达有边界（v1.33）：正文超过 120 字或含换行不直达（交启发式提取由模型裁决，避免截断产生无意义片段）；疑问语气（结尾 吗/呢/？/?）不直达；与画像既有内容规范化比对重复时不写入。
...
- **画像白盒读写与结构化命中（v1.30）**：
  - `get_assistant_profile(assistant_id) -> str` / `replace_assistant_profile(assistant_id, content) -> None`：整文读写 `profile.md`，UTF-8 原样写入、不重排既有格式、不自动补写头部；`content` 超过 `ASSISTANT_PROFILE_MAX_CHARS`（50,000 字符）抛 `ValueError`，空白内容属显式清空、原样落盘；写入走临时文件 + 原子替换（v1.33），失败按原内容尽力回滚，不留下半程状态（与 persona 编辑同模式）。读写仍只经 MemoryStore，前端只能经 API 访问（§5.9），禁止直接读助手目录。
...
| 编辑运行中的助手（PATCH） | 先取助手运行锁，任一任务运行期间拒绝（409），边界与删除一致；写失败按原内容尽力回滚，不留下半程状态，回滚也失败时经日志告警（v1.33）；写入走临时文件 + 原子替换（v1.33）（§4.2/§5.9） |
