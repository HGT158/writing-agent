...
> 版本：v1.17 · 2026-08-14
> 状态：阶段 4 写作 IDE、v1.11 复审加固、v1.13 项目 Agent 流式编辑、v1.14 空白文档生成修复、v1.15 项目 Agent 多会话历史、v1.16 失败路径加固及 v1.17 活动流跨事件滑窗、内联 diff 审阅与上下文压缩均已完成；SSE 网络断线后的游标续传暂缓
...
**活动连接的事件寻址必须使用单调递增序号，不能使用列表下标**（v1.17）：事件历史是有界滑窗，一次流式回复的 `token` 事件数量很容易超过窗口容量。每个事件在记录时分配任务内唯一且递增的 `seq`；活动订阅者从独立队列收取事件，并用自身游标跳过已经发送的序号。窗口裁剪不得导致活动订阅者停止收流，也不得吞掉 `task_done` / `task_failed` 终态事件。当前 HTTP SSE 接口尚未接收 `Last-Event-ID` 或显式 `after_seq`，前端在 `EventSource.onerror` 时也会关闭连接，因此网络断线后的自动续传尚未实现，见 `docs/guides/backlog.md`。所有返回 202 的任务创建端点必须在入队前校验助手存在且当前没有有效运行锁；未知助手返回 404，已忙返回 409，不得先创建注定失败的任务或在异步错误中泄漏助手列表。`api.main` 只提供 `create_app` 工厂；生产入口为 `api.server:app`，避免导入 API 模块时打开真实数据库。局部改写与聊天沿用任务流；聊天文本 delta 使用 `token`，工具开始/终结使用 `tool_call` / `tool_result`，修改建议使用 `change_preview`（含 `change_set_id`、项目/文档 id、原文范围、建议文本和基准版本）。正文只有在 apply 成功后更新。
...
SSE 下发的每个事件都带任务内单调递增的 `seq`（示例中省略），活动订阅者据此跨越事件滑窗；网络断线后的重放游标尚未接入，见 §5.9。
...
| SSE 网络断线 | 当前前端关闭 EventSource 并提示刷新，服务端任务仍可能继续；尚不支持按最后 `seq` 自动续传，后续实现见 `docs/guides/backlog.md` |
