...
**修复建议**（已用只读片段验证正确）：切片改为 `match.string[match.start():match.end() - len(value)]`。先补 RED 用例：长前缀（s > 值长）场景断言敏感值的**任意子串**（如 `"sk-abcdef" not in detail`）不出现、且匹配后文本不重复；同时把现有用例的断言从「全串不在」收紧为「敏感值前缀子串不在」。
...
1. **P1-1 最优先**：一行修复，但必须先补长前缀泄漏的 RED 用例并收紧现有断言，再改切片表达式；在修复前，失败详情通道的值级脱敏应视为未生效。
...
- **P1-1 已闭环**：捕获组分支切片终点修正为 `match.end() - len(value)`，并按本报告建议先补长前缀（s > 值长）RED 用例、把既有 detail 脱敏断言从"完整密钥串不在"收紧为"敏感值任意前缀子串不得出现"；修复后另行用 conda 环境独立复现本报告的原始泄漏样例，确认 `api_key=***`、无子串泄漏、无尾部文本重复。
