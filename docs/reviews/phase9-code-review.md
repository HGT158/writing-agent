...
## 处理结果记录（2026-08-24，v1.27）

本节记录第一梯队加固闭环；上文发现、行号与 `228/117` 测试数保留为 2026-08-23 审查时点的历史事实，不回写覆盖。

- **P0-1 已闭环**：`applyAllChanges` 在任何接受请求前收集批次内全部已打开且 dirty 的目标文档，按文档去重列出相对路径并只确认一次；取消时整批零请求。前端用例覆盖多文档清单与取消分支。
- **P1-3 已闭环**：MemoryStore 共享 SQLite 连接以 `isolation_level=None` 打开；简单 DML autocommit，不再跨方法遗留隐式事务，多语句写路径继续显式 `BEGIN IMMEDIATE` 并保留既有 rollback。用例验证连接口径、单语句后无未决事务及后续显式事务可正常开始。
- **P1-1 已闭环**：`ToolRegistry` 拒绝重复名称并返回实际注册数；Runtime 先保留内置工具、跳过同名 MCP 工具并发 warning，启动信息按成功注册的 MCP 数量统计。用例同时验证 `read_file` 内置实现未被覆盖、warning 与 `内置 3 + MCP 1` 计数。
- **P1-4 已闭环**：`redact` 对对象/数组中的每个字符串叶子调用值级密钥扫描，敏感键整值替换规则与非 JSON 普通文本边界不变；用例覆盖 args/result 内非敏感键下的 `sk-`、Bearer 与 `token=value`。
- **P1-7 已闭环**：FastAPI 增加 `TrustedHostMiddleware`，生产仅允许 `127.0.0.1` / `localhost`（端口不限），测试工厂额外允许 `testserver`；恶意 Host 返回 400。本仓库 Vite `/api` 代理为字符串简写，默认 `changeOrigin=false`，会透传浏览器本机 Host，无需修改配置；用例以 `127.0.0.1:5173` 验证开发地址通过。
